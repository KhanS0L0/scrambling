...
public class B7B9 {
    ...
    //в качестве полинома был взят B[i] = A[i] ⊕ B[i-7] ⊕ B[i-9]
    ...
    public static void main(String[] args) {
        ...
        for(int i =0; i < 96; i++){
            if(i >= 9){
                b1 = result.substring(i-7, i-6);
                b2 = result.substring(i-9, i-8);
                a = str.substring(i, i+1);
                result.append(a.charAt(0) - '0' ^ b1.charAt(0) - '0' ^ b2.charAt(0) - '0');
                System.out.println("B[" +(i+1)+ "] = A[" +(i+1)+ "] ^ B[i-7] ^ B[i-9] = " + result.charAt(i));
            }else if(i >= 7){
                a = str.substring(i, i+1);
                b1 = result.substring(i-7, i-6);
                result.append(a.charAt(0)^b1.charAt(0));
                System.out.println("B[" +(i+1)+ "] = A[" +(i+1)+ "] ^ B[i-7] = " + result.charAt(i));
            }else{
